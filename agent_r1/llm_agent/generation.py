...
class ToolGenerationManager:
    ...
    def run_llm_loop(self, gen_batch, env: Union[BaseToolEnv, BaseImageToolEnv]) -> Tuple[Dict, Dict]:
        ...
        successes = torch.zeros(batch_size, dtype=torch.int32)
        ...
        for _ in range(self.config.max_turns):
            ...
            if isinstance(env, BaseToolEnv):
                if self.config.use_batch_tool_calls:
                    tool_responses, tool_successes, new_active_masks = env.batch_step(raw_responses)
                    success_counts = [sum(s) for s in tool_successes]
                    successes[active_mask] += torch.tensor(success_counts, dtype=torch.int32)
                else:
                    tool_responses = []
                    new_active_masks = []
        ...
        final_output['successes'] = successes
