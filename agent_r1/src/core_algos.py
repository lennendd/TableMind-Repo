...
def compute_rank_grpo_advantage(
    ...
    with torch.no_grad():
        ...
        id2indices = defaultdict(list) # Helper to track original indices per group
        
        # Group scores and indices by prompt
        for i in range(bsz):
            id2score[index[i]].append(scores[i])
            id2indices[index[i]].append(i)

        # Calculate base advantage for each response
        base_advantages = torch.zeros_like(scores)
        for idx in id2score:
            group_scores = torch.tensor(id2score[idx], device=device)
            group_original_indices = id2indices[idx]
            
            if len(group_scores) > 1:
                mean = group_scores.mean()
                std = group_scores.std()
            else:
                # For a single-item group, advantage is 0 as it's its own mean.
                mean = group_scores[0] 
                std = torch.tensor(1.0, device=device)

            if norm_adv_by_std_in_grpo:
                group_adv = (group_scores - mean) / (std + epsilon)
            else: # Dr.GRPO style
                group_adv = group_scores - mean
                
            base_advantages[group_original_indices] = group_adv

        # Calculate per-sequence log_prob
        seq_len = action_mask.sum(dim=-1)
        seq_len = torch.clamp(seq_len, min=1)
        seq_log_prob = (log_prob * action_mask).sum(dim=-1) / seq_len
        
        # Initialize gamma factors to 1 (no change by default)
        gamma_factors = torch.ones_like(scores)
        
        # Iterate through each group to perform pairwise comparison
        for idx in id2score.keys():
            group_original_indices = torch.tensor(id2indices[idx], device=device)
            
            # We can only form pairs if there are at least 2 responses in the group
            if len(group_original_indices) < 2:
                continue
                
            group_scores = scores[group_original_indices]
            group_log_probs = seq_log_prob[group_original_indices]
            
            # Identify winners and losers *within this group*
            winner_mask = group_scores > reward_threshold
            loser_mask = ~winner_mask
            
            # Get the indices relative to the group
            winner_indices_in_group = winner_mask.nonzero(as_tuple=True)[0]
            loser_indices_in_group = loser_mask.nonzero(as_tuple=True)[0]
            
            # If no winners or losers in the group, we can't form pairs
            if len(winner_indices_in_group) == 0 or len(loser_indices_in_group) == 0:
                continue
                
            # Select the log_probs for winners and losers
            winner_log_probs = group_log_probs[winner_indices_in_group]
            loser_log_probs = group_log_probs[loser_indices_in_group]
            
            # Use broadcasting to compute all pairwise log_prob differences
            # log_prob_diffs shape: (num_winners, num_losers)
            log_prob_diffs = winner_log_probs.unsqueeze(1) - loser_log_probs.unsqueeze(0)
            
            # Find inverted pairs where the difference is less than the margin
            is_inverted = (log_prob_diffs < margin).float()
            
            # Calculate the gamma weight for each pair
            pair_gammas = 1.0 + alpha * is_inverted
            
            # Aggregate gammas for each winner and loser
            if pair_gammas.numel() > 0:
                gamma_w = pair_gammas.mean(dim=1) # Average over losers for each winner
                gamma_l = pair_gammas.mean(dim=0) # Average over winners for each loser

                # Get the original batch indices for this group's winners and losers
                original_winner_indices = group_original_indices[winner_indices_in_group]
                original_loser_indices = group_original_indices[loser_indices_in_group]
                
                # Update the global gamma_factors tensor
                gamma_factors[original_winner_indices] = gamma_w
                gamma_factors[original_loser_indices] = gamma_l

        # Apply the rank-aware weighting to the base advantages
        final_advantages = base_advantages * gamma_factors
        
        # Broadcast per-sequence advantages to per-token advantages
        final_advantages_per_token = final_advantages.unsqueeze(-1).expand_as(token_level_rewards) * action_mask

        # The original function returns two identical tensors. We follow that.
    return final_advantages_per_token, final_advantages_per_token
