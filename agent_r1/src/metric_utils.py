...
def compute_data_metrics(batch: DataProto, use_critic: bool = True) -> Dict[str, Any]:
    ...
    if "successes" in batch.batch:
        successes = batch.batch["successes"].float()
        tool_users_mask = (turns > 0)
        tool_usage_ratio = torch.mean(tool_users_mask.float()).detach().item()

        total_turns = torch.sum(turns)
        if total_turns > 0:
            total_successes = torch.sum(successes)
            tool_success_rate = (total_successes / total_turns).detach().item()
        else:
            # 如果没有任何工具调用，则成功率为0
            tool_success_rate = 0.0

        metrics.update({
            "tool/usage_ratio": tool_usage_ratio,
            "tool/success_rate": tool_success_rate,
            "tool/total_successes": torch.sum(successes).detach().item(),
            "tool/total_turns": total_turns.detach().item(),
        })
    ...
    if "tool_call_score" in batch.non_tensor_batch:
        tool_call_score = batch.non_tensor_batch["tool_call_score"]
        metrics.update(
            {
                # tool_call score
                "critic/tool_call_score/mean": np.mean(tool_call_score),
                "critic/tool_call_score/max": np.max(tool_call_score),
                "critic/tool_call_score/min": np.min(tool_call_score),
            }
        )
